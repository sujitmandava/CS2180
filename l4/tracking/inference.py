...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            # print(x,y)
            # variableDomainsDict[OBS0].append(x+y+7)
            # variableDomainsDict[OBS0].append(x+y)
            # variableDomainsDict[OBS1].append(x+y+7)
            # variableDomainsDict[OBS1].append(x+y)
            variableDomainsDict[PAC].append((x,y))
            variableDomainsDict[GHOST0].append((x,y))
            variableDomainsDict[GHOST1].append((x,y)) 
    
    for i in range(X_RANGE+Y_RANGE+6):
